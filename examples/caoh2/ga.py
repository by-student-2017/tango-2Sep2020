...
def singlepoint(t, kptdensity=3.5):
    ...
    try:
        calc = DftbPlusCalculator(t, kpts=kptdensity, use_spline=True,
                              maximum_angular_momenta={'Ca': 0, 'H': 0, 'O': 1})
...
def relax_one(t, kptdensity=3.5):
    ...
    t.set_constraint(c)
    blmin = {(1, 1): 1.8, (1, 8): 0.9, (1, 20): 1.8, (8, 8): 2.0,
             (8, 20): 1.5, (20, 20): 1.5}
    ...
    calc = DftbPlusCalculator(kpts=kptdensity, use_spline=True, read_chg=True,
                              maximum_angular_momenta={'Ca': 0, 'H': 0, 'O': 1})
...
def run_ga(n_to_test, kptdensity=3.5):
    ...
    #permustrainmut = PermuStrainMutation(permut, strainmut)
...
def prepare_ga(dbfile='godb.db', splits={(2,): 1}, N=20):
    blocks = [('Ca', 4), ('OH', 8)]  # the building blocks
