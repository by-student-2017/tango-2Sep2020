...
def singlepoint(t, kptdensity=3.5):
    if get_raw_score(t) < -1e5:
        return t
    try:
        calc = DftbPlusCalculator(kpts=kptdensity, use_spline=True,
                                  maximum_angular_momenta={'Pd': 2, 'O': 1})
...
def relax_one(t, kptdensity=3.5):
    ...
    clock = time()
    t.wrap()
    calc = DftbPlusCalculator(kpts=0.66*kptdensity,
    ...
    del t.constraints
    t.wrap()
    calc = DftbPlusCalculator(kpts=kptdensity,
                              use_spline=True, read_chg=True,
                              maximum_angular_momenta={'Pd': 2, 'O': 1})
...
def run_ga(n_to_test, kptdensity=3.5):
    ...
    softmut = SoftMutation(blmin_soft, bounds=[2., 5.],
                           fconstfunc=fconstfunc, use_tags=False)
...
def prepare_ga(dbfile='godb.db', splits={(2,): 1}, N=20):
    ...
    blocks = [('Pd', 6), ('O', 6)]  # the building blocks
    volume = 10. * 12 # volume in angstrom^3
