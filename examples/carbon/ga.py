...
def relax_one(t, kptdensity=3.5):
    ...
    clock = time()
    t.wrap()
    calc = DftbPlusCalculator(t, kpts=0.66*kptdensity, use_spline=True,
                              read_chg=True, maximum_angular_momenta={'C': 1})
...
def prepare_ga(dbfile='godb.db', splits={(2,): 1}, N=20):
    ...
    blocks = [('C', 12)]  # the building blocks
    volume = 8. * 12 # volume in angstrom^3
